...
var heart_spawner = preload("res://heart_icon.tscn")
...
@onready var key_d: TutorialKey = %KeyD
...
func _input(event: InputEvent) -> void:
	if (event is InputEventKey and event.pressed):
		if (event.keycode == Key.KEY_A):
			key_a.highlight()
		elif (event.keycode == Key.KEY_SPACE):
			key_space.highlight()
		elif (event.keycode == Key.KEY_D):
			key_d.highlight()
	elif (event is InputEventKey and not event.pressed):
		if (event.keycode == Key.KEY_A):
			key_a.unhighlight()
		elif (event.keycode == Key.KEY_SPACE):
			key_space.unhighlight()
		elif (event.keycode == Key.KEY_D):
			key_d.unhighlight()
...
func _on_tourist_hit(_tourist: Tourist):
	...
	lives -= 1
	var life_to_remove = lives_container.get_child(lives) as HeartIcon
	life_to_remove.blink_away()
...
func _reset_stats() -> void:
	...
	for i in range(lives):
		var heart_instance = heart_spawner.instantiate()
