...
func _spawn_tourist_group():
    var group_size = randi_range(1, 5) # Groups of 1-5 tourists
    ...
    var base_y = randf_range(-2.0, 2.0) # Random lane position
    
    for i in range(group_size):
        var tourist_instance = tourist_spawner.instantiate()
